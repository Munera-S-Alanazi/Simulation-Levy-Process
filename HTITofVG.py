...
from ParametersOfVG import compute_expression, compute_norms, find_K, find_M_h, search_x0_xK, f, df_dz

# The characteristic function as in Cont & Tankov book
def vgCF(t, z, theta, sigma, kappa):
    i = complex(0.0, 1.0)  # Define the imaginary unit
    term = 1.0 - i * z * theta * kappa + 0.5 * sigma**2 * z**2 * kappa
    return term**(-t / kappa)
...
def hilbert_transform_cdf(x, t, theta, sigma, kappa, h, M):
    ...
    for m in range(-M, M + 1):
        z = (m - 0.5) * h
        term = np.exp(-1j * z * x) * vgCF(t, z, theta, sigma, kappa) / (((m - 0.5) * np.pi))
        summation += term
...
t = 4.0
d_plus = (theta*kappa+ np.sqrt((theta**2)*kappa**2 +(2*(sigma**2)*kappa))) / (kappa*sigma**2)
d_minus =  (theta*kappa - np.sqrt((theta**2)*kappa**2 +(2*(sigma**2)*kappa))) / (kappa*sigma**2)

print(f"|d_+ = {d_plus}")
print(f"|d_- = {d_minus}")

norm_plus, norm_minus = compute_norms(vgCF, d_plus, d_minus, t, theta, sigma, kappa, 1e-6)
print(f"norm + = {norm_plus}")
print(f"norm - = {norm_minus}")

decay_kappa = (sigma**2 * kappa / 2) ** (-t / kappa) 
nu = 2*t/kappa  
...
cf_real_theoretical = [vgCF(t, z, theta, sigma, kappa) for z in zValues]
real_parts_theoretical = [val.real for val in cf_real_theoretical]
n_f = 0  # Number of non-differentiable points
h_vals = np.linspace(5, 0.01, 50)  #  range for h to improve convergence
M_vals = np.arange(1, 400, 1)  #  range for M with a step
...
for epsilon in epsilon_values:
    ...
    x_min, x_max = search_x0_xK(f, vgCF, d_plus, d_minus, t, theta, sigma, kappa, epsilon)
    x_min_values.append(x_min)
    x_max_values.append(x_max)
    
    K = find_K(df_dz, vgCF, x_min, x_max, epsilon, t, theta, sigma, kappa)
    K_values.append(K)
    ...
    M, h = find_M_h(f, df_dz, compute_expression, x_min, x_max, t, theta, sigma, kappa, d_minus, d_plus, norm_minus, norm_plus, K, n_f, epsilon, h_vals, M_vals, nu, decay_kappa)
    ...
    plt.title(f"Real Component of Characteristic Function of VG\n x_min = {x_min:.4f}, x_max = {x_max:.4f}, K = {K}, h = {h:.4f}, M = {M}")
